...
return dojo.declare(
	...
	null,
	{
		...
		//	TODO:
		//		-	using 0 && console.log() in a test prints something on the
		...
		//			function(el, index) {0 && console.log(el, index)}) that works on
		...
		_run: function(/*Array*/tests){
			...
			for(var i=0; i<len; i++){
				...
				if(res.success){
					...
					0 && console.info(msg+"OK: "+viewCommands);
					oks += 1;
				}else{
					this.errors.push({
						commands: t.commands,
			...
			0 && console.info(len+" tests ran.", oks+" Success,", this.errors.length+" Errors");
		},
