...
dojo.declare("dojox.xmpp.ChatService", null, {
	...
	sendMessage: function(msg){
		if (!this.uid){
			//0 && console.log("ChatService::sendMessage() -  Contact Id is null, need to invite to chat");
			return;
		}
...
/*
		if (msg.xhtml){
			if (msg.xhtml.getAttribute('xmlns') != dojox.xmpp.xmpp.XHTML_IM_NS){
				//0 && console.log("ChatService::sendMessage() - Cannot use this xhtml without the propper xmlns");
			}else{
				//FIXME do this in some portable way
				//0 && console.log("ChatService::sendMessage() - FIXME Serialize XHTML to string: ", msg.xhtml.toString());
			}
		}
*/
