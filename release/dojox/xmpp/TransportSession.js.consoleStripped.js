...
dojo.extend(dojox.xmpp.TransportSession, {
		...
		processScriptSrc: function(msg, rid) {
			//0 && console.log("processScriptSrc::", rid, msg);
		//	var msgDom = dojox.xml.DomParser.parse(msg);
			var msgDom = dojox.xml.parser.parse(msg, "text/xml");
			//0 && console.log("parsed mgs", msgDom);
			//0 && console.log("Queue", this.outboundQueue);
			if(msgDom) {
				this.processDocument(msgDom, rid);
			} else {
				//0 && console.log("Received bad document from server",msg);
			}
		},
		...
		//	//0 && console.log("In dispatchPacket ", msg, protocolMatchType, matchId, matchProperty);
			if (msg){
		...
		_dispatchPacket: function(){
			...
			if (!this.sid){
				0 && console.debug("TransportSession::dispatchPacket() No SID, packet dropped.")
				return;
			}
			...
			if (!this.authId){
				...
				0 && console.debug("TransportSession::dispatchPacket() No authId, packet dropped [FIXME]")
				return;
			}
			...
			if (this.protocolPacketQueue.length > 0){
				...
				//0 && console.log("Nothing to send, I'm just polling.");
				if(this.lastPollTime) {
					var now = new Date().getTime();
					if(now - this.lastPollTime < this.polling) {
						//0 && console.log("Waiting to poll ", this.polling - (now - this.lastPollTime)+10);
		...
		sendXml: function(message, rid){
			...
			//0 && console.log("TransportSession::sendXml()"+ new Date().getTime() + " RID: ", rid, " MSG: ", message);
			this.transmitState = "transmitting";
			var def = null;
			if(this.useScriptSrcTransport) {
				//0 && console.log("using script src to transmit");
				...
				def = dojo.rawXhrPost({
					...
					handleAs: "xml",
					error: dojo.hitch(this, function(res, io) {
						////0 && console.log("foo", res, io.xhr.responseXML, io.xhr.status);
		...
		processDocument: function(doc, rid){
			...
			//0 && console.log("TransportSession:processDocument() ", doc, rid);
			this.transmitState = "idle";

			var body = doc.firstChild;
			if (body.nodeName != 'body'){
				//0 && console.log("TransportSession::processDocument() firstChild is not <body> element ", doc, " RID: ", rid);
			}

			if (this.outboundQueue.length<1){return false;}

			var expectedId = this.outboundQueue[0]["rid"];
			//0 && console.log("expectedId", expectedId);
			if (rid==expectedId){
				this.removeFromOutboundQueue(rid);
				this.processResponse(body, rid);
				this.processInboundQueue();
			}else{
				//0 && console.log("TransportSession::processDocument() rid: ", rid, " expected: ", expectedId);
				var gap = rid-expectedId;
			
				if (gap < this.hold + 2){
					this.addToInboundQueue(doc,rid);
				}else{
					//0 && console.log("TransportSession::processDocument() RID is outside of the expected response window");
				}
			}
			return doc;
		},
		...
		processResponse: function(body,rid){
			////0 && console.log("TransportSession:processResponse() ", body, " RID: ", rid);
			...
			if ((this.state != 'Ready')&&(this.state != 'Terminate')) {
				...
				//0 && console.log("Polling value ", this.polling);
		...
		processError: function(err, httpStatusCode,rid){
			//0 && console.log("Processing server error ", err, httpStatusCode,rid);
			if(this.isTerminated()) {
				return false;
			}
			...
			if (err && err.dojoType && err.dojoType=="timeout"){
				//0 && console.log("Wait timeout");
			}
			
			this.removeFromOutboundQueue(rid);
			//FIXME conditional processing if request will be needed based on type of error.
			if(err && err.firstChild) {
			//0 && console.log("Error ", err.firstChild.getAttribute("type") + " status code " + httpStatusCode);
