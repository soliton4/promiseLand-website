...
dojo.extend(dojox.xmpp.xmppSession, {
		...
		processProtocolResponse: function(msg){
			//0 && console.log("xmppSession::processProtocolResponse() ", msg, msg.nodeName);
			...
			switch(type){
				...
				case "features":
					...
					break;
				default:
					//0 && console.log("default action?", msg.getAttribute('xmlns'));
		...
		messageHandler: function(msg){
			//0 && console.log("xmppSession::messageHandler() ",msg);
		...
		iqHandler: function(msg){
			//0 && console.log("xmppSession::iqHandler()", msg);
		...
		presenceHandler: function(msg){
			//0 && console.log("xmppSession::presenceHandler()");
			switch(msg.getAttribute('type')){
				case 'subscribe':
					//0 && console.log("PresenceHandler: ", msg.getAttribute('from'));
				...
				case 'error':
					...
					//0 && console.log("xmppService::presenceHandler() Error");
					break;
				default:
		...
		featuresHandler: function(msg){
			//0 && console.log("xmppSession::featuresHandler() ",msg);
			var authMechanisms = [];
			...
			if(msg.hasChildNodes()){
				for(var i=0; i<msg.childNodes.length;i++){
					...
					//0 && console.log("featuresHandler::node", n);
					switch(n.nodeName){
						case 'mechanisms':
							for (var x=0; x<n.childNodes.length; x++){
								//0 && console.log("featuresHandler::node::mechanisms", n.childNodes[x].firstChild.nodeValue);
			...
			//0 && console.log("Has connected/bind?", this.state, hasBindFeature, authMechanisms);
				...
				if(!this.auth){
					...
					for(var i=0; i<authMechanisms.length; i++){
						try{
							...
							0 && console.warn("No suitable auth mechanism found for: ", authMechanisms[i]);
						}
					}
		...
		saslHandler: function(msg){
			//0 && console.log("xmppSession::saslHandler() ", msg);
		...
		chatHandler: function(msg){
			//0 && console.log("xmppSession::chatHandler() ", msg);
			var message = {
			...
			var chatState = null;
				//0 && console.log("chat child node ", msg.childNodes, msg.childNodes.length);
			for (var i=0; i<msg.childNodes.length; i++){
				var n = msg.childNodes[i];
				if (n.hasChildNodes()){
					//0 && console.log("chat child node ", n);
					switch(n.nodeName){
						case 'thread':
						...
						case 'x':
							break;
						default:
							//0 && console.log("xmppSession::chatHandler() Unknown node type: ",n.nodeName);
					}
				}
				/*//0 && console.log("Foo", n, n.nodeName);
			...
			if (message.chatid){
				for (var i=0; i< this.chatRegister.length; i++){
					...
					////0 && console.log("ci.chatid: ", ci.chatid, message.chatid);
		...
		simpleMessageHandler: function(msg){
			//0 && console.log("xmppSession::simpleMessageHandler() ", msg);
		},
		...
		rosterSetHandler: function(elem){
			//0 && console.log("xmppSession::rosterSetHandler()", arguments);
			var r;
		...
		presenceUpdate: function(msg){
			if(msg.getAttribute('to')){
				...
				if(jid != this.jid) {
					//0 && console.log("xmppService::presenceUpdate Update Received with wrong address - ",jid);
					return;
				}
			}
		...
		retrieveRoster: function(){
			////0 && console.log("xmppService::retrieveRoster()");
			var props={
				id: this.getNextIqId(),
				from: this.jid + "/" + this.resource,
				type: "get"
		...
		createRosterEntry: function(elem){
			////0 && console.log("xmppService::createRosterEntry()");
			var re = {
...
*/
		...
		dispatchPacket: function(msg, type, matchId){
			if (this.state != "Terminate") {
				...
				//0 && console.log("xmppSession::dispatchPacket - Session in Terminate state, dropping packet");
			}
		},
		...
		_onSearchResults: function(msg){
			if ((msg.getAttribute('type')=='result')&&(msg.hasChildNodes())){
				//0 && console.log("xmppSession::_onSearchResults(): ", msg.firstChild);
		...
		onLogin: function(){
			////0 && console.log("xmppSession::onLogin()");
			this.retrieveRoster();
		},

		onLoginFailure: function(msg){
			//0 && console.log("xmppSession::onLoginFailure ", msg);
		},

		onBindResource: function(msg, hasSession){
			//0 && console.log("xmppSession::onBindResource() ", msg);

			if (msg.getAttribute('type')=='result'){
				//0 && console.log("xmppSession::onBindResource() Got Result Message");
				if ((msg.hasChildNodes()) && (msg.firstChild.nodeName=="bind")){
					...
					//0 && console.log("xmppService::onBindResource() No Bind Element Found");
				}

				this.onLogin();

			}else if(msg.getAttribute('type')=='error'){
				//0 && console.log("xmppSession::onBindResource() Bind Error ", msg);
		...
		onBindSession: function(msg){
			if(msg.getAttribute('type')=='error'){
				//0 && console.log("xmppSession::onBindSession() Bind Error ", msg);
				var err = this.processXmppError(msg);
				this.onLoginFailure(err);
			}else{
				this.onLogin();
		...
		onSearchResults: function(results){
			//0 && console.log("xmppSession::onSearchResult() ", results);
		},

		onRetrieveRoster: function(msg){
			////0 && console.log("xmppService::onRetrieveRoster() ", arguments);
			...
			if ((msg.getAttribute('type')=='result') && msg.hasChildNodes()){
				...
				//0 && console.log("xmppService::storeRoster()  Error received on roster get");
			}

			////0 && console.log("Roster: ", this.roster);
		...
		onTransportReady: function(){
			...
			////0 && console.log("xmppSession::onTransportReady()");
		},
		...
		onConnected: function(){
			////0 && console.log("xmppSession::onConnected()");
		},

		onTerminate: function(newState, oldState, message){
			//0 && console.log("xmppSession::onTerminate()", newState, oldState, message);
		},

		onActive: function(){
			////0 && console.log("xmppSession::onActive()");
			//this.presenceService.publish({show: dojox.xmpp.presence.STATUS_ONLINE});
		},

		onRegisterChatInstance: function(chatInstance, message){
			////0 && console.log("xmppSession::onRegisterChatInstance()");
		},
		...
		processXmppError: function(msg){
			////0 && console.log("xmppSession::processXmppError() ", msg);
			var err = {
		...
		sendStanzaError: function(stanzaType,to,id,errorType,condition,text){
			////0 && console.log("xmppSession: sendStanzaError() ", arguments);
			var req = {type:'error'};
			if (to) { req.to=to; }
			if (id) { req.id=id; }
