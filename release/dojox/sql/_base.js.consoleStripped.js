...
define("dojox/sql/_base", ["dojo","dijit","dojox","dojo/require!dojox/sql/_crypto"], function(dojo,dijit,dojox){
...
dojo.mixin(dojox.sql, {
	...
	_exec: function(params){
		try{
			...
			0 && console.debug("SQL Exception: " + exp);
			
			if(this._autoClose){
				try{
					this.close();
				}catch(e){
					0 && console.debug("Error closing database: "
									+ e.message||e);
				}
			}
...
dojo.declare("dojox.sql._SQLCrypto", null, {
	...
	_encrypt: function(sql, password, args, encryptColumns, callback){
		//0 && console.debug("_encrypt, sql="+sql+", password="+password+", encryptColumns="+encryptColumns+", args="+args);
	...
	_decrypt: function(resultSet, needsDecrypt, password, callback){
		//0 && console.debug("decrypt, resultSet="+resultSet+", needsDecrypt="+needsDecrypt+", password="+password);
	...
	_decryptSingleColumn: function(columnName, columnValue, password, currentRowIndex,
											callback){
		//0 && console.debug("decryptSingleColumn, columnName="+columnName+", columnValue="+columnValue+", currentRowIndex="+currentRowIndex)
		dojox.sql._crypto.decrypt(columnValue, password, dojo.hitch(this, function(results){
			...
			if(this._finishedCrypto >= this._totalCrypto
				&& this._finishedSpawningCrypto){
				//0 && console.debug("done with all decrypts");
